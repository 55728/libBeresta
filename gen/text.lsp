...
"))
  ...
  \\endparblock")
	...
	(:название "Page_TextMatrix"
	 ...
	 :результат (:тип "TransMatrix"
		     :ru "Матрица трансформации текста \\ref BRST_TransMatrix при успехе, иначе возвращается `BRST_TransMatrix{1, 0, 0, 1, 0, 0}`."
	...
	(:название "Page_TextLeading"
	 ...
	 :ошибки ()
	 :результат (:тип "REAL"
		     :ru "Текущий межстрочный интервал при успехе, иначе возвращается \\c 0."
	...
	(:название "Page_TextRenderingMode"
	 ...
	 :результат (:тип "TextRenderingMode"
		     :ru "Текущий режим рендеринга текста при успехе, иначе возвращается \\ref BRST_FILL."
	...
	(:название "Page_TextRise"
	 ...
	 :ошибки ()
	 :результат (:тип "REAL"
		     :ru "Текущее смещение текста по вертикали при успехе, иначе возвращается \\c 0."
	...
	(:название "Page_CharSpace"
	 ...
	 :ошибки ()
	 :результат (:тип "REAL"
		     :ru "Текущее межсимвольное расстояние при успехе, иначе возвращается \\c 0."
	...
	(:название "Page_WordSpace"
	 ...
	 :ошибки ()
	 :результат (:тип "REAL"
		     :ru "Текущее межсловное расстояние при успехе, иначе возвращается \\c 0."
	...
	(:название "Page_CurrentTextPos"
	 ...
	 :ошибки ()
	 :результат (:тип "Point"
		     :ru "Текущие координаты отображения текста в \\ref BRST_Point при успехе, иначе возвращается `BRST_Point{0, 0}`."
	...
	(:название "Page_CurrentTextPos2"
	 ...
	 :параметры ((:тип "Page"
                     ...
                     (:тип "Point*"
                      :имя "pos"
                      :ru "Указатель на структуру, куда записывается текущая позиция."
	...
	(:название "Page_CurrentFont"
	 ...
	 :ошибки ()
	 :результат (:тип "Font"
		     :ru "Текущий выбранный шрифт при успехе, иначе возвращается \\c NULL."
	...
	(:название "Page_CurrentFontSize"
	 ...
	 :ошибки ()
	 :результат (:тип "REAL"
		     :ru "Текущий размер шрифта при успехе, иначе возвращается \\c 0."
	...
	(:название "Page_MeasureText"
	 ...
	 :ошибки ("BRST_INVALID_PAGE"
		  "BRST_PAGE_FONT_NOT_FOUND")
	 :результат (:тип "UINT"
		     :ru "Количество байт, помещающихся в заданную ширину"
		     :en "Byte length that fits within specified width.")
	 :ru "Измеряет количество символов, которое поместится в заданную область с учётом переноса слов."
	 :en "Measure how many bytes fit within specified width considering word wrapping."
	 :параметры ((:тип "Page"
                      :имя "page"
                      :ru ":param_page"
                      :en ":param_page")
  ...
  \\image html img/measuretext.png")
                     (:тип "REAL*"
                      :имя "real_width"
                      :ru "Указатель на переменную для хранения реальной ширины текста."
